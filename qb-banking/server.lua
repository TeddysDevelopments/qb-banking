...
local function generateCardNumber()
    ...
    local letters = ""
    for i = 1, 4 do
        letters = letters .. generateRandomLetter() 
    end
...
QBCore.Functions.CreateCallback('qb-banking:server:openATM', function(source, cb)
    ...
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return end
    local result = MySQL.Sync.fetchAll('SELECT card_pin FROM player_cards WHERE citizenid = @citizenid', {
        ['@citizenid'] = citizenid
    })
    local acceptablePins = {}
    if result[1] and result[1].card_pin then
        table.insert(acceptablePins, result[1].card_pin)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.no_pin_set'), 'error')
    end
    local job = Player.PlayerData.job
    local gang = Player.PlayerData.gang
    local accounts = {}
...
local function generateUniqueCardNumber()
    local cardNumber = generateCardNumber()
    local result = MySQL.Sync.fetchAll('SELECT card_number FROM player_cards WHERE card_number = @cardNumber', {
...
QBCore.Functions.CreateCallback('qb-banking:server:orderCard', function(source, cb, data)
    ...
    local result = MySQL.Sync.fetchAll('SELECT card_number FROM player_cards WHERE citizenid = @citizenid', {
        ['@citizenid'] = citizenid
    })

    if result[1] then
        local cardNumber = result[1].card_number
        MySQL.Async.execute('UPDATE player_cards SET card_pin = @cardPin WHERE citizenid = @citizenid AND card_number = @cardNumber', {
            ['@citizenid'] = citizenid,
            ['@cardPin'] = pinNumber,
            ['@cardNumber'] = cardNumber
        }, function(updatedRows)
            if updatedRows > 0 then
                cb({ success = true, message = Lang:t('Pin Changed') })
            else
                cb({ success = false, message = Lang:t('could not change pin') })
            end
        end)
    else
        local cardNumber = generateUniqueCardNumber() 
        MySQL.Async.execute('INSERT INTO player_cards (citizenid, name, card_number, card_pin) VALUES (@citizenid, @name, @cardNumber, @cardPin)', {
            ['@citizenid'] = citizenid,
            ['@name'] = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname,
            ['@cardNumber'] = cardNumber,
            ['@cardPin'] = pinNumber,
        }, function(insertAffectedRows)
            if insertAffectedRows > 0 then
