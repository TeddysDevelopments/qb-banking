const bankingApp = Vue.createApp({
    ...
    methods: {
        ...
        orderDebitCard() {
            // Ensure the PIN is not empty and is exactly 4 digits
            if (this.debitPin.length !== 4 || this.debitPin.length < 4) {
                this.addNotification("Please enter a valid 4-digit PIN.", "error");
                return;
            }
        
            // Proceed with the API request if the PIN is valid (exactly 4 digits)
            axios
                ...
                .then((response) => {
                    if (response.data.success) {
                        this.debitPin = ""; // Clear the PIN after a successful order
                        this.addNotification(response.data.message, "success");
                    } else {
                        this.addNotification(response.data.message, "error");
                    }
                ...
                .catch((error) => {
                    this.addNotification("An error occurred. Please try again.", "error");
                });
        },
        openAccount() {
